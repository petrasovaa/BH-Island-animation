...
do
    ...
    r.mapcalc "new2 = if (isnull(inner), if(new == 1, 1, null()), 1)" --o --q
    r.to.vect new2 type=area output=new3 -s --o --q
    v.clean input=new3 output=new4 tool=rmarea thres=10000 -c --o --q
    if [[ "$E" =~ coast* ]]; then
        g.rename vector=new3,new5 --o 
    else
        echo $E
        v.generalize input=new4 output=new5 method=reumann thre=5 --o --q
        v.generalize input=new5 output=$E method=boyle thre=400 --o --q
    fi
...
EOF
    ...
    d.rast new
    d.vect $E fcolor=none color=98:85:36 width=3 --q
...
convert -delay 1x1  `ls elevation* -tr` -coalesce -layers OptimizeTransparency animation.gif
